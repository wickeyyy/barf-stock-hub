...
local lastGearStock = {}
local lastEggStock = {}
local sentWatchlistPings = {} -- track sent pings to avoid duplicates

-- ══════════════════════════════════════════════
--   EDIT ROLE IDs HERE
--   Replace YOUR_ROLE_ID with actual Discord role ID
--   To get role ID: Developer Mode ON → right-click role → Copy Role ID
-- ══════════════════════════════════════════════
local ROLE_PINGS = {
    ["Super Fertilizer"]     = "<@&YOUR_ROLE_ID>",
    ["Super Pet Treat"]      = "<@&YOUR_ROLE_ID>",
    ["Radioactive Spray"]    = "<@&YOUR_ROLE_ID>",
    ["Rainbow Spray"]        = "<@&YOUR_ROLE_ID>",
    ["Cosmic Spray"]         = "<@&YOUR_ROLE_ID>",
    ["Prismatic Fertilizer"] = "<@&YOUR_ROLE_ID>",
    ["Bubblegum Spray"]      = "<@&YOUR_ROLE_ID>",
    ["Fire Spray"]           = "<@&YOUR_ROLE_ID>",
    ["Epic Egg"]             = "<@&YOUR_ROLE_ID>",
}
-- ══════════════════════════════════════════════

-- ── Watchlist ─────────────────────────────────────────────────────────────────
local WATCHLIST = {}
for k in pairs(ROLE_PINGS) do WATCHLIST[k] = true end
...
-- ── Item images (proxy URL for Discord compatibility) ─────────────────────────
local function assetThumb(id)
    local num = tostring(id):match("%d+")
    return "https://www.roblox.com/asset-thumbnail/image?assetId=" .. num .. "&width=420&height=420&format=png"
end

local GEAR_IMAGES = {
    ["Normal Fertilizer"]    = assetThumb("108748897686608"),
    ["Strong Fertilizer"]    = assetThumb("139969851411479"),
    ["Super Fertilizer"]     = assetThumb("81550812764026"),
    ["Acid Spray"]           = assetThumb("113512432518563"),
    ["Wet Spray"]            = assetThumb("136245425357255"),
    ["Frozen Spray"]         = assetThumb("123943839236644"),
    ["Void Spray"]           = assetThumb("129850208807511"),
    ["Radioactive Spray"]    = assetThumb("108030220388601"),
    ["Rainbow Spray"]        = assetThumb("87225134160383"),
    ["Autumn Spray"]         = assetThumb("81243026423073"),
    ["Cosmic Spray"]         = assetThumb("72678246007745"),
    ["Normal Pet Treat"]     = assetThumb("110118415860907"),
    ["Strong Pet Treat"]     = assetThumb("136469591048144"),
    ["Super Pet Treat"]      = assetThumb("130065517672591"),
    ["Bubblegum Spray"]      = assetThumb("119042092688782"),
    ["Fire Spray"]           = assetThumb("121926441629482"),
    ["Prismatic Fertilizer"] = assetThumb("128223583670977"),
}
...
-- ── Watchlist ping (no duplicates) ───────────────────────────────────────────
local function sendWatchlistPing(item, shopName)
    -- prevent duplicate pings within same restock cycle
    if sentWatchlistPings[item.name] then return end
    sentWatchlistPings[item.name] = true

    local r, price = getGearRarity(item.name)
    if item.rarity then r = item.rarity end
    if item.price and item.price ~= "???" then price = item.price end
    local color = RI[r] or RI.Common
    local ping = ROLE_PINGS[item.name] or "@here"

    local embed = {
        title = "🚨 WATCHLIST ALERT — " .. item.name,
        description = (RE[r] or "⚪") .. " **" .. item.name .. "** is now in stock!\n" ..
            "Rarity: `" .. r .. "` · Price: " .. price ..
            (item.stock and "\nStock: **" .. item.stock .. "**" or ""),
        color = color,
    ...
    local payload = HttpService:JSONEncode({
        content = ping .. " 🚨 **" .. item.name .. "** is in stock!",
        embeds = {embed}
    })
            ...
            if podium then
                ...
                if sg2 then
                    ...
                    if eggLabel and eggLabel.Text ~= "" then
                        local r, price = getEggRarity(eggLabel.Text)
                        table.insert(results, {
                            slot=i, name=eggLabel.Text,
                            price=priceLabel and priceLabel.Text or price,
                            rarity=r
                        })
                    end
                end
...
local function sendAutoGear()
    ...
    for _,item in ipairs(items) do
        ...
        table.insert(lines, (RE[item.rarity] or "⚪").." **"..item.name.."**"..star.." `"..item.rarity.."` · "..item.price.." · Stock: "..item.stock)
        for _,tier in ipairs(tierOrder) do
            if item.rarity==tier then
                if (RI[tier] or 0) > topColor then topColor=RI[tier] if not firstNotable then firstNotable=item.name end end
                break
            end
        end
    end
    ...
    local embed = {
        title = "⚙️ Gear Shop Stock",
        description = "Restocked · " .. os.date("%H:%M"),
        color = topColor,
        fields = {{name="In Stock ("..#items..")", value=table.concat(lines,"\n"), inline=false}},
        footer = {text="Build A Ring Farm Stock"},
        timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ"),
    }
    if firstNotable and GEAR_IMAGES[firstNotable] then
        embed.thumbnail = {url = GEAR_IMAGES[firstNotable]}
    end
    ...
    for _,item in ipairs(items) do
        if WATCHLIST[item.name] then
            sendWatchlistPing(item, "Gear Shop")
        end
    end
end

local function sendAutoEgg()
    if WEBHOOK_URL=="" then return end
    ...
    for _,item in ipairs(items) do
        local star = WATCHLIST[item.name] and " ⭐" or ""
        table.insert(lines, "**Slot "..item.slot.."** · "..(RE[item.rarity] or "⚪").." **"..item.name.."**"..star.." `"..item.rarity.."` · "..item.price)
        for _,tier in ipairs(tierOrder) do
    ...
    local embed = {
        title = "🥚 Egg Shop Stock",
        description = "Restocked · " .. os.date("%H:%M"),
        color = topColor,
        fields = {{name="Egg Slots ("..#items.."/5)", value=table.concat(lines,"\n"), inline=false}},
        footer = {text="Build A Ring Farm Stock"},
        timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ"),
    }
    httpPost(WEBHOOK_URL, HttpService:JSONEncode({embeds={embed}}))
    ...
    for _,item in ipairs(items) do
        if WATCHLIST[item.name] then
            sendWatchlistPing(item, "Egg Shop")
        end
    end
end
...
gSendBtn.MouseButton1Click:Connect(function()
    ...
    for _,item in ipairs(GEAR_LIST) do
        if gChecked[item.name] then
            local star=WATCHLIST[item.name] and " ⭐" or ""
            table.insert(lines,(RE[item.rarity] or "⚪").." **"..item.name.."**"..star.." `"..item.rarity.."` · "..item.price)
            for _,tier in ipairs(tierOrder) do
                if item.rarity==tier then
                    if (RI[tier] or 0)>topColor then topColor=RI[tier] if not firstNotable then firstNotable=item.name end end
                    break
    ...
    task.spawn(function()
        local embed={title="⚙️ Gear Shop Stock",description="Updated · "..os.date("%H:%M"),color=topColor,
            fields={{name="In Stock ("..#lines..")",value=table.concat(lines,"\n"),inline=false}},
            footer={text="Build A Ring Farm Stock"},timestamp=os.date("!%Y-%m-%dT%H:%M:%SZ")}
        if firstNotable and GEAR_IMAGES[firstNotable] then embed.thumbnail={url=GEAR_IMAGES[firstNotable]} end
...
eSendBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        local embed={title="🥚 Egg Shop Stock",description="Updated · "..os.date("%H:%M"),color=topColor,
...
autoInfo.Size=UDim2.new(1,-16,0,70) autoInfo.Position=UDim2.new(0,8,0,30)
autoInfo.BackgroundTransparency=1
autoInfo.Text="Scans every 5 min · Sends only when stock changes\n⭐ Watchlist items trigger role pings with images\nNo duplicate pings per restock cycle\n\nWatchlist: Super Fertilizer, Super Pet Treat,\nRadioactive/Rainbow/Cosmic/Prismatic/Bubblegum/Fire Spray, Epic Egg"
...
autoScanNow.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        lastGearStock={} lastEggStock={} sentWatchlistPings={}
        sendAutoGear() sendAutoEgg()
        autoLastLbl.Text="Last scan: "..os.date("%H:%M:%S")
...
wTest.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        local ok,err=httpPost(WEBHOOK_URL,HttpService:JSONEncode({
            embeds={{title="✅ Build A Ring Farm Stock — Test",description="Webhook connected!\n⭐ Role pings + images active.",color=3066993,footer={text="Build A Ring Farm Stock"}}}
        }))
...
task.spawn(function()
    while true do
        ...
        -- Reset ping tracker on each restock so pings fire again next cycle
        sentWatchlistPings={}
...
print("[Build A Ring Farm Stock v4] Loaded!")
