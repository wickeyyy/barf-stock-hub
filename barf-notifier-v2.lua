...
local AUTO_INTERVAL = 30 -- seconds
local lastGearStock = {}
local lastEggStock = {}
...
tl("BARF Stock Hub",14,C.Text,30,5) tl("Auto + Manual Reporter v6",10,C.Sub,30,22)
...
local function sendWebhook(selectedItems,shopName,allList)
    ...
    local payload=HttpService:JSONEncode({
        ...
        embeds={{
            ...
            color=topColor,
            fields={{name="Available Items ("..#items..")",value=table.concat(lines,"\n"),inline=false}},
            footer={text="BARF Stock Hub v6 • Auto+Manual"},
...
local function scanGearShop()
    ...
    local ok = pcall(function()
        ...
        for _, child in pairs(sf:GetChildren()) do
            ...
            if rarityLabel then
                local stockText = rarityLabel.Text -- "Stock: 3" or "Stock: 0"
                local stock = tonumber(stockText:match("Stock: (%d+)")) or 0
                if stock > 0 then
...
local function sendAutoGear()
    ...
    for _,item in ipairs(items) do
        ...
        table.insert(lines, emoji.." **"..item.name.."** `"..r.."` · "..price.." · Stock: "..item.stock)
    end

    local payload=HttpService:JSONEncode({
        content=hasRare and "🚨 **Rare gear in stock!**" or nil,
        embeds={{
            title="⚙️ BARF — Gear Shop Stock (Auto)",
            description="Auto scan · "..os.date("%H:%M"),
            color=topColor,
            fields={{name="In Stock ("..#items..")",value=table.concat(lines,"\n"),inline=false}},
            footer={text="BARF Stock Hub v6 • Auto Scan"},
            timestamp=os.date("!%Y-%m-%dT%H:%M:%SZ"),
        }}
    })
...
local function sendAutoEgg()
    ...
    for _,item in ipairs(items) do
        ...
        table.insert(lines, "**Slot "..item.slot.."** · "..emoji.." **"..item.name.."** `"..r.."` · "..item.price)
    end

    local payload=HttpService:JSONEncode({
        content=hasRare and "🚨 **Rare egg in stock!**" or nil,
        embeds={{
            title="🥚 BARF — Egg Shop Stock (Auto)",
            description="Auto scan · "..os.date("%H:%M"),
            color=topColor,
...
for i,item in ipairs(GEAR_LIST) do makeCheckRow(gSc,item,i,gChecked,nil) end
...
gAutoBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if #items > 0 then
            ...
            for _,c in ipairs(gSc:GetChildren()) do
                if c:IsA("Frame") then
                    local nameLbl=c:FindFirstChildOfClass("TextLabel")
                    local cb=c:FindFirstChildOfClass("TextButton")
                    if nameLbl and cb then
                        local itemName=nameLbl.Text:gsub("[^%a%s]",""):gsub("^%s+",""):gsub("%s+$","")
                        if gChecked[itemName] then
            ...
            gAutoBtn.Text="⚠️ Nothing found / Shop closed"
        end
...
-- ── EGG PAGE ──────────────────────────────────────────────────────────────────
local EP=makePage()
hdr(EP,"  EGG SHOP — 5 slots (walk to egg merchant)",0)
...
for slot=1,5 do
    ...
    local function updateSlot(forceIdx)
        if forceIdx then idx=forceIdx end
    ...
    table.insert(slotBtns,{btn=eggBtn,updateFn=updateSlot,eggOptions=eggOptions})
end
    ...
    task.spawn(function()
        local payload=HttpService:JSONEncode({
            ...
            embeds={{
                ...
                footer={text="BARF Stock Hub v6 • Manual"},
                timestamp=os.date("!%Y-%m-%dT%H:%M:%SZ"),
            }}
        })
        ...
        if #items > 0 then
            for _,item in ipairs(items) do
                ...
                if sf then
                    ...
                    local eb = sf:FindFirstChildOfClass("TextButton")
                    if eb then
                        eb.Text=(RE[r] or "⚪").." "..item.name
                        eb.TextColor3=RC[r] or C.Sub eb.BackgroundColor3=C.BG
                    end
                end
            end
...
-- ── AUTO PAGE ─────────────────────────────────────────────────────────────────
local AP=makePage()
hdr(AP,"  AUTO NOTIFIER — scans every 5 minutes",0)
...
autoInfo.Size=UDim2.new(1,-16,0,40) autoInfo.Position=UDim2.new(0,8,0,30)
autoInfo.BackgroundTransparency=1
autoInfo.Text="Scans Gear Shop + Egg Shop every 5 minutes.\nOnly sends to Discord when stock changes.\nRequires webhook to be set first."
...
autoToggle.MouseButton1Click:Connect(function()
    if not AUTO_ENABLED then
        ...
        AUTO_ENABLED=true
        autoToggle.Text="⏹  Stop Auto Notifier"
        autoToggle.BackgroundColor3=C.Red
        autoStatusLbl.Text="🟢 Auto Notifier: ON"
        autoStatusLbl.TextColor3=C.Green
        ...
        autoToggle.Text="▶  Start Auto Notifier"
        autoToggle.BackgroundColor3=C.Green
        autoStatusLbl.Text="🔴 Auto Notifier: OFF"
        autoStatusLbl.TextColor3=C.Red
...
autoScanNow.MouseButton1Click:Connect(function()
    if WEBHOOK_URL=="" then
        ...
        task.delay(2,function() autoScanNow.Text="🔍  Scan & Send Now" end) return
    end
    autoScanNow.Text="⏳ Scanning..."
    task.spawn(function()
        lastGearStock={} lastEggStock={} -- force send
        sendAutoGear()
        sendAutoEgg()
        autoLastLbl.Text="Last scan: "..os.date("%H:%M:%S")
...
wTest.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        local ok,err=httpPost(WEBHOOK_URL,HttpService:JSONEncode({
            embeds={{title="✅ BARF Stock Hub v6 — Test",description="Webhook working! Auto+Manual reporter ready.",color=3066993,footer={text="BARF Stock Hub v6"}}}
        }))
...
task.spawn(function()
    while true do
        ...
        -- Auto scan on restock if enabled
        if AUTO_ENABLED and WEBHOOK_URL~="" then
            task.wait(3) -- wait for shop to update
            lastGearStock={} lastEggStock={} -- force send on restock
            sendAutoGear()
            sendAutoEgg()
...
print("[BARF Stock Hub v6] Loaded! Auto+Manual reporter ready.")
